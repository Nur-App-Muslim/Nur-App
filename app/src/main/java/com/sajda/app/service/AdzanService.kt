...
class AdzanService : Service() {
    ...
    private suspend fun triggerAdzan(
        ...
        var alarmVolumeAudible = audioManager.getStreamVolume(AudioManager.STREAM_ALARM) > 0

        if (!alarmVolumeAudible && settings.overrideSilentMode) {
            runCatching {
                val maxVolume = audioManager.getStreamMaxVolume(AudioManager.STREAM_ALARM)
                val targetVolume = (maxVolume * 0.75).toInt().coerceAtLeast(1)
    ...
    private fun createNotification(
        ...
        val notifTitle = getString(com.sajda.app.R.string.displayname_time_has_arrived).replace("$displayName", displayName)

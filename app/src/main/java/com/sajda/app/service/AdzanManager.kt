...
class AdzanManager(private val context: Context) {
    ...
    suspend fun checkAndUpdateLocation(): Boolean {
        return try {
            ...
            val allSchedules = prayerTimeRepository.getNextDaysPrayerTimes(days = 30)
                .ifEmpty { prayerTimeRepository.refreshPrayerTimes(settings, days = 30) }
            adzanScheduler.reschedule(allSchedules, settings)
            Log.d(TAG, "Alarm adzan berhasil di-set ulang untuk kota $newCity dengan ${allSchedules.size} hari jadwal")
            true

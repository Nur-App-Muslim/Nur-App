...
class AdzanScheduler @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    suspend fun reschedule(
        ...
        var upcomingPrayers = findUpcomingScheduledPrayers(prayerTimes, settings, referenceTime)
        if (upcomingPrayers.isEmpty()) {
            val prayerTimeRepository = PrayerTimeRepository(SajdaDatabase.getDatabase(appContext))
            val freshTimes = prayerTimeRepository.refreshPrayerTimes(settings, days = 30)
            upcomingPrayers = findUpcomingScheduledPrayers(freshTimes, settings, referenceTime)
        }
    ...
    companion object {
        ...
        suspend fun repairNextAlarm(
            ...
            if (prayerTimes.size < 7 || prayerTimes.none { !LocalDate.parse(it.date).isBefore(referenceTime.toLocalDate()) }) {

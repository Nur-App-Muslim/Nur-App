...
function Resolve-ApkSigner {
    ...
    if (-not $sdkRoot -and (Test-Path (Join-Path $RootDir "local.properties"))) {
        $sdkLine = Get-Content (Join-Path $RootDir "local.properties") | Where-Object { $_ -match "^sdk\.dir=" } | Select-Object -First 1
        if ($sdkLine) {
...
try {
    ...
    Ensure-GradleSetting "compileSdk\s*=\s*\d+" "compileSdk harus ditentukan."
    Ensure-GradleSetting "minSdk\s*=\s*21" "minSdk harus 21."
    Ensure-GradleSetting "targetSdk\s*=\s*\d+" "targetSdk harus ditentukan."
